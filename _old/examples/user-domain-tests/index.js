...
describe('user-domain example', () => {
  const testEventFlow = async container => {
    ...
    expect(userCreatedEvent).to.have.property('aggregateId').that.is.not
      .undefined
    expect(userCreatedEvent).to.have.property('aggregateVersion', 0)
    expect(userCreatedEvent).to.have.property('type', 'userCreated')
    expect(userCreatedEvent).to.have.nested.property(
      'payload.username',
      'sherlock'
    )
    expect(userCreatedEvent)
      .to.have.nested.property('payload.passwordHash')
      .that.does.not.eq('magic')

    // created user aggregateId can be retrieved from "userCreated" event
    const userAggregateId = userCreatedEvent.aggregateId
    ...
    expect(userPasswordChanged)
      .to.have.nested.property('payload.passwordHash')
      .that.does.not.eq('no-magic')
  }
  ...
  describe('with DI container', () => {
    ...
    it('updates Users projection view', () => testProjection(createContainer()))
  })
