...
describe('CommandBus', function() {
  ...
  describe('on(commandType, handler)', () => {
    ...
    it('sets up a handler on messageBus for a given commandType', () => {
      ...
      expect(messageBus.on).to.have.nested.property(
        'firstCall.args[0]',
        'doSomething'
      )
      expect(messageBus.on)
        .to.have.nested.property('firstCall.args[1]')
        .that.is.a('Function')
    })
  })
  ...
  describe('sendRaw(command)', () => {
    ...
    it('passes a formatted command to messageBus', () => {
      ...
      return bus.sendRaw(command).then(() => {
        expect(messageBus.send).to.have.nested.property(
          'lastCall.args[0]',
          command
        )
      })
  ...
  describe('send(commandType, aggregateId, options)', () => {
    ...
    it('validates parameters', () => {
      expect(() => bus.send(undefined)).to.throw(
        'type argument must be a non-empty String'
      )
      expect(() => bus.send('test', 1)).to.throw(
        'options argument must be an Object'
      )
      expect(() => bus.send('test', 1, {}, {}, {})).to.throw(
        'more than expected arguments supplied'
      )
    })
    ...
    it('formats a command and passes it to sendRaw', async () => {
      ...
      expect(bus.sendRaw).to.have.nested.property(
        'lastCall.args[0].aggregateId',
        aggregateId
      )
      expect(bus.sendRaw).to.have.nested.property(
        'lastCall.args[0].context',
        context
      )
      expect(bus.sendRaw).to.not.have.nested.property(
        'lastCall.args[0].payload'
      )
      ...
      expect(bus.sendRaw).to.have.nested.property(
        'lastCall.args[0].customParameter',
        customParameter
      )
    })
