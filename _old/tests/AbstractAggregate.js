...
describe('AbstractAggregate', function() {
  ...
  describe('constructor(id, state, events)', () => {
    it('throws exception if event handler is not defined', () => {
      ...
      expect(() => new AggregateWithoutHandler({ id: 1 })).to.throw(
        "'somethingHappened' handler is not defined or not a function"
      )
    })
  })
    ...
    it('throws error, if command handler is not defined', async () => {
      try {
        ...
        expect(err).to.have.property(
          'message',
          "'doSomethingUnexpected' handler is not defined or not a function"
        )
      }
    })
  ...
  describe('mutate(event)', () => {
    ...
    it('does not mutate state if state event handler is not defined', () => {
      ...
      assert(
        state.somethingHappened.calledOnce,
        'somethingHappened handler was not called once'
      )

      expect(() =>
        agg.mutate({ type: 'somethingStatelessHappened' })
      ).to.not.throw()
    })
    ...
    it('restores aggregate version and snapshotVersion, when snapshot event provided', () => {
      ...
      expect(agg).to.have.property(
        'snapshotVersion',
        snapshotEvent.aggregateVersion
      )
      expect(agg).to.have.property(
        'version',
        snapshotEvent.aggregateVersion + 1
      )
    })
  })
  ...
  describe('restoreSnapshot(snapshotEvent)', () => {
    ...
    it('validates arguments', () => {
      ...
      for (const keyToMiss of Object.keys(snapshotEvent)) {
        ...
        const brokenEvent = JSON.parse(
          JSON.stringify(snapshotEvent, keysToCopy)
        )
      ...
      ).to.throw('snapshot event type expected')
    ...
    it('restores aggregate state from a snapshot', () => {
      ...
      expect(agg)
        ...
        .that.deep.equals(snapshotEvent.payload)
    })
  })
})
