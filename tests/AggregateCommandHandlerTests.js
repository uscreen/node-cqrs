...
describe('AggregateCommandHandler', function() {
  ...
  it('restores aggregate from event store events', async () => {
    ...
    assert(
      eventStore.getAggregateEvents.calledOnce,
      'getAggregateEvents was not called'
    )
  ...
  it('resolves to produced events', async () => {
    ...
    const events = await handler.execute({
      type: 'doSomething',
      aggregateId: 1
    })
  ...
  it.skip('executes concurrent commands on same aggregate instance', async () => {
    ...
    const handler = new AggregateCommandHandler({
      eventStore,
      aggregateType: PersistedAggregate
    })
    ...
    expect(storage).to.have.nested.property(
      'saveAggregateSnapshot.callCount',
      0
    )
